...
console.log('🕋 100% HALAL EXNESS TRADING BOT - REAL AI');
console.log('📦 Version: 20.0.0');
console.log('🤖 AI Mode: Continuous Analysis (EVERY SECOND)');
console.log('🔑 Primary API Key is set.');
...
async function fetchBalance(accountId) {
    try {
        ...
        if (error.message.includes('401') || error.message.includes('Unauthorized') || error.message.includes('403')) {
            console.error('⚠️ API KEY EXPIRED OR INVALID!');
...
async function getRealAIDecision(symbol, accountId) {
    try {
        ...
        const momentum1h = prices.length >= 60 ? ((prices[prices.length - 1] - prices[prices.length - 60]) / (prices[prices.length - 60] || 1)) * 100 : 0;
        ...
        // Support & Resistance
        const support = fib.level382 || Math.min(...prices.slice(-20));
        const resistance = fib.level618 || Math.max(...prices.slice(-20));
        ...
        // RSI based scoring
        if (rsi < 25) { buyScore += 35; reasons.push(`Extreme oversold RSI ${rsi.toFixed(1)}`); }
        else if (rsi < 35) { buyScore += 25; reasons.push(`Oversold RSI ${rsi.toFixed(1)}`); }
        else if (rsi > 75) { sellScore += 35; reasons.push(`Extreme overbought RSI ${rsi.toFixed(1)}`); }
        else if (rsi > 65) { sellScore += 25; reasons.push(`Overbought RSI ${rsi.toFixed(1)}`); }
        ...
        if (bb.lower && currentPrice <= bb.lower) { buyScore += 20; reasons.push('At lower Bollinger Band'); }
        else if (bb.upper && currentPrice >= bb.upper) { sellScore += 20; reasons.push('At upper Bollinger Band'); }
        
        // Momentum scoring
        if (momentum > 0.3) { buyScore += 10; reasons.push(`Strong momentum ${momentum.toFixed(2)}%`); }
        else if (momentum > 0.1) { buyScore += 5; reasons.push(`Positive momentum ${momentum.toFixed(2)}%`); }
        else if (momentum < -0.3) { sellScore += 10; reasons.push(`Strong negative momentum ${momentum.toFixed(2)}%`); }
        else if (momentum < -0.1) { sellScore += 5; reasons.push(`Negative momentum ${momentum.toFixed(2)}%`); }
        
        // Divergence scoring
        if (divergence === 'BULLISH') { buyScore += 30; reasons.push('Bullish divergence detected'); }
        else if (divergence === 'BEARISH') { sellScore += 30; reasons.push('Bearish divergence detected'); }
        
        // Fibonacci scoring
        if (currentPrice <= fib.level382 * 1.005) { buyScore += 15; reasons.push('At Fibonacci support'); }
        if (currentPrice >= fib.level618 * 0.995) { sellScore += 15; reasons.push('At Fibonacci resistance'); }
        
        // Sentiment scoring
        if (sentiment > 0.65) { buyScore += 10; reasons.push(`Bullish sentiment ${(sentiment*100).toFixed(0)}%`); }
        else if (sentiment < 0.35) { sellScore += 10; reasons.push(`Bearish sentiment ${(sentiment*100).toFixed(0)}%`); }
        
        // Volume scoring
        if (volumeSpike && sentiment > 0.5) { buyScore += 10; reasons.push('High volume with bullish action'); }
        else if (volumeSpike && sentiment < 0.5) { sellScore += 10; reasons.push('High volume with bearish action'); }
        ...
        console.log(`🤖 AI DECISION [${symbol}]: ${action} (${(confidence*100).toFixed(0)}%)`);
        console.log(`   Buy Score: ${buyScore}, Sell Score: ${sellScore}`);
        console.log(`   ${reasons.slice(0, 3).join(' | ')}`);
...
async function shouldClosePositionAI(position, accountId) {
    try {
        ...
        // AI Profit Taking Logic
        if (profitPercent > 0) {
            // Take profit based on market conditions
            if (profitPercent >= 3) {
                shouldClose = true;
                reason = `High profit ${profitPercent.toFixed(2)}% - taking profits`;
                closeConfidence = 0.95;
            } else if (profitPercent >= 1.5) {
                // Check for reversal signals
                if ((position.side === 'buy' && rsi > 70) || (position.side === 'sell' && rsi < 30)) {
                    shouldClose = true;
                    reason = `Profit ${profitPercent.toFixed(2)}% with overbought/oversold RSI`;
                    ...
                    closeConfidence = 0.8;
                } else if ((position.side === 'buy' && bb.upper && currentPrice >= bb.upper) || 
                          (position.side === 'sell' && bb.lower && currentPrice <= bb.lower)) {
                    shouldClose = true;
                    reason = `Profit ${profitPercent.toFixed(2)}% at Bollinger Band extreme`;
                    closeConfidence = 0.8;
                } else if ((position.side === 'buy' && macd.histogram < 0) || 
                          (position.side === 'sell' && macd.histogram > 0)) {
                    shouldClose = true;
                    reason = `Profit ${profitPercent.toFixed(2)}% with MACD reversal`;
                    closeConfidence = 0.75;
                }
            }
            
            // Trail profit if profit is good but no reversal yet
            if (!shouldClose && profitPercent >= 1) {
                ...
                if (profitPercent > maxProfit) {
                    ...
                    shouldClose = true;
                    reason = `Profit retraced from ${maxProfit.toFixed(2)}% to ${profitPercent.toFixed(2)}%`;
                    closeConfidence = 0.7;
                }
            }
            ...
            if (loss >= 2) {
                ...
                // Check if loss is accelerating
                if ((position.side === 'buy' && momentum < -0.1) || (position.side === 'sell' && momentum > 0.1)) {
                    shouldClose = true;
                    reason = `Loss ${loss.toFixed(2)}% with accelerating momentum against position`;
                    closeConfidence = 0.85;
                }
            }
        }
        ...
        if (shouldClose) {
            console.log(`🎯 AI CLOSE DECISION: ${position.symbol} | ${reason}`);
        }
        
        return { shouldClose, reason, profitPercent, currentPrice, closeConfidence };
    } catch (error) {
        console.error('❌ AI close decision error:', error.message);
...
class HalalTradingEngine {
    ...
    async start() {
        console.log(`🕋 Starting AI Halal trading engine for ${this.userEmail}`);
        ...
        // AI Analysis EVERY SECOND - Continuous market monitoring
    ...
    async executeTrade(symbol, side, aiDecision) {
        ...
        try {
            ...
            console.log(`   Reason: ${aiDecision.reasons[0]}`);
            console.log(`   ${aiDecision.buyScore > aiDecision.sellScore ? 'Buy Score' : 'Sell Score'}: ${Math.max(aiDecision.buyScore, aiDecision.sellScore)}`);
    ...
    async closePosition(position, profitPercent, currentPrice) {
        try {
            ...
            console.log(`✅ AI CLOSED ${position.symbol} | Profit: ${profitSymbol}$${profit.toFixed(2)} (${profitPercent.toFixed(2)}%) | Total: $${this.currentProfit.toFixed(2)}`);
        } catch (error) {
    ...
    async stop() {
        console.log(`🛑 Stopping AI trading engine for ${this.userEmail}`);
        this.isActive = false;
...
app.post('/api/start-trading', authenticate, async (req, res) => {
    try {
        ...
        res.json({ 
            success: true, 
            sessionId, 
            message: `✅ AI HALAL TRADING STARTED! Investment: $${investmentAmount} | Target: $${targetProfit} | 🤖 AI analyzes EVERY SECOND using 10+ indicators!` 
        });
    } catch (error) {
...
app.listen(PORT, '0.0.0.0', () => {
    console.log(`\n🕋 100% HALAL EXNESS TRADING BOT - REAL AI`);
    ...
    console.log(`✅ 🤖 REAL AI: RSI, MACD, Bollinger Bands, Support/Resistance, Divergence, Fibonacci, Momentum, Sentiment, Volatility`);
